...
public class ApduCommandManager {
    ...
    public ApduCommandManager(MfiChipHolder chipHolder) throws UnsupportedOperationException, IllegalArgumentException {
        LogMgr.log(3, "000");
        if (chipHolder == null) {
    ...
    public void sendCommand(JSONObject cmdApdu) throws JSONException, InterruptedException, IllegalArgumentException, GpException {
        LogMgr.log(3, "000");
        this.mApduResponse = null;
        doSendCommand(cmdApdu);
        LogMgr.log(3, "999");
    }
    ...
    private void doSendCommand(final JSONObject cmdApdu) throws JSONException, InterruptedException, IllegalArgumentException, GpException {
        LogMgr.log(4, "000");
        try {
            if (cmdApdu == null) {
            ...
            try {
                try {
                    try {
                        ApduResponse apduResponse = new ApduResponse(cmdApdu);
    ...
    private boolean select(byte[] aid) throws InterruptedException, IllegalArgumentException, GpException {
        byte[] bArrSelect;
        LogMgr.log(6, "000");
        try {
            LogMgr.log(6, "001");
            bArrSelect = this.mGpController.select(aid);
        } catch (GpException e) {
            ...
            if (response != null) {
                ...
                if (!selectResponse.isStatusSuccess()) {
                    ...
                    this.mApduResponse.setErrorMessage(ObfuscatedMsgUtil.omapiExecutionPoint("Channel#getSelectResponse", "SW=" + StringUtil.bytesToHexString(selectResponse.getSw())));
                    LogMgr.log(6, "998");
                    return false;
                }
                bArrSelect = response;
        ...
        if (aid != null) {
            ...
            if (bArrSelect.length > 2048) {
                ...
                System.arraycopy(bArrSelect, 0, bArr, 0, 2048);
                this.mApduResponse.setSelectResponse(bArr);
                LogMgr.log(1, "803 Response length is over 2048bytes.");
                throw new GpException(225, ObfuscatedMsgUtil.executionPoint(), ObfuscatedMsgUtil.omapiExecutionPoint("Channel#getSelectResponse", "ResponseTooLong"));
            }
    ...
    private boolean transmit(byte[] command) throws JSONException, InterruptedException, IllegalArgumentException, GpException {
        LogMgr.log(6, "000");
        if (command == null) {
        ...
        try {
            byte[] bArrTransmit = this.mGpController.transmit(command);
            ...
            if (!responseAnalyzer.isStatusSuccess()) {
                ...
                this.mApduResponse.setErrorMessage(ObfuscatedMsgUtil.omapiExecutionPoint("Channel#transmit", "SW=" + StringUtil.bytesToHexString(responseAnalyzer.getSw())));
                return false;
            }
            if (bArrTransmit.length > 2048) {
                byte[] bArr = new byte[2048];
                System.arraycopy(bArrTransmit, 0, bArr, 0, 2048);
                LogMgr.log(1, "803 Response length is over 2048bytes.");
                this.mApduResponse.setApduResponse(bArr);
                throw new GpException(225, ObfuscatedMsgUtil.executionPoint(), ObfuscatedMsgUtil.omapiExecutionPoint("Channel#transmit", "ResponseTooLong"));
            }
            this.mApduResponse.setApduResponse(bArrTransmit);
            LogMgr.log(6, "999");
            return true;
    ...
    private static class ApduResponse {
        ...
        private ApduResponse(JSONObject cApduJson) throws JSONException {
            this.mCurrentBlockId = 0;
            this.mCurrentCmdId = 0;
            this.mIsRunApdu = false;
            LogMgr.log(3, "000");
            this.mResult = new JSONObject(cApduJson.toString());
            LogMgr.log(3, "999");
        }
        ...
        void setRunApduFlag(boolean isRunApdu) {
            this.mIsRunApdu = isRunApdu;
        }

        void setCurrentBlockId(int idx) throws JSONException, IllegalArgumentException {
            LogMgr.log(3, "000 idx: " + idx);
            if (idx < 0) {
                LogMgr.log(1, "800 idx = " + idx);
                throw new IllegalArgumentException();
            }
            this.mCurrentBlockId = idx;
            setCurrentCmdId(0);
            LogMgr.log(3, "999");
        }

        void setCurrentCmdId(int idx) throws JSONException, IllegalArgumentException {
            LogMgr.log(3, "000 idx: " + idx);
            if (idx < 0) {
                LogMgr.log(1, "800 idx = " + idx);
                throw new IllegalArgumentException();
            }
            this.mCurrentCmdId = idx;
            refresh();
        ...
        void setApduResponse(byte[] response) throws IllegalArgumentException {
            String strBytesToHexString = StringUtil.bytesToHexString(response);
        ...
        void setSelectResponse(byte[] response) throws IllegalArgumentException {
            String strBytesToHexString = StringUtil.bytesToHexString(response);
            LogMgr.log(3, "000 response: " + strBytesToHexString);
            try {
        ...
        void setErrorMessage(String message) throws IllegalArgumentException {
            LogMgr.log(3, "000 message: " + message);
            try {
                this.mApduBlock.put("errorMessage", message);
                update();
                LogMgr.log(3, "999");
            } catch (JSONException unused) {
                throw new IllegalArgumentException();
        ...
        /* JADX DEBUG: Move duplicate insns, count: 1 to block B:5:0x0010 */
