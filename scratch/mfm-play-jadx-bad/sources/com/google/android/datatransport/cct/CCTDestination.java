...
public final class CCTDestination implements EncodedDestination {
    ...
    public byte[] asByteArray() {
        ...
        return String.format("%s%s%s%s", EXTRAS_VERSION_MARKER, str2, EXTRAS_DELIMITER, str).getBytes(Charset.forName(StringUtil.UTF_8));
    }

    public static CCTDestination fromByteArray(byte[] bArr) {
        String str = new String(bArr, Charset.forName(StringUtil.UTF_8));
    ...
    static byte[] encodeString(String str) {
        return str.getBytes(Charset.forName(StringUtil.UTF_8));
    }

    static String decodeExtras(byte[] bArr) {
        return new String(bArr, Charset.forName(StringUtil.UTF_8));
    }
}
