...
public class Initializer {
                ...
                try {
                    try {
                        try {
                            ...
                            Initializer.this._runner.putFailure(new ModelErrorInfo(ModelErrorInfo.Type.OTHER_ERROR, new MfmException(Initializer.class, 1, e)));
                            asyncRunner = Initializer.this._runner;
                            notify = new FuncUtil.Notify() { // from class: com.felicanetworks.mfm.main.model.internal.main.Initializer.1.1
                                @Override // com.felicanetworks.mfm.main.model.internal.main.FuncUtil.Notify
                                public void go() {
    ...
    public void checkVersionUp(NetworkExpert networkExpert, DatabaseExpert databaseExpert) {
        ...
        try {
            ...
            try {
                VersionUpProtocol.Result result = versionUpProtocol.parse(networkExpert.connect(requestCreate));
                this._runner.checkInterrupted();
                ...
                if (i2 == 0) {
                    databaseExpert.setUpgradeSettings(new DatabaseExpert.UpgradeSettings(result.offlineCheckInterval, FuncUtil.convertFromCalendarToString(DateFormatter.DATE_MINUTE, vuParams.current), 0, result.offlineCheckMaxCnt));
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void checkTosVersionUp(NetworkExpert networkExpert, DatabaseExpert databaseExpert) {
        VuParams vuParams = new VuParams();
        try {
            DatabaseExpert.TosVersionSettings tosVersionSettings = databaseExpert.getTosVersionSettings();
            int i = tosVersionSettings.chkCnt;
            tosVersionSettings.chkCnt = i + 1;
            vuParams.factCount = i;
            vuParams.maxCount = tosVersionSettings.maxCnt;
            databaseExpert.setTosVersionSettings(tosVersionSettings);
            if (vuParams.setup(tosVersionSettings)) {
                TosVersionProtocol tosVersionProtocol = networkExpert.getTosVersionProtocol();
                ...
                this._runner.checkInterrupted();
                try {
                    this._tosVersionProtocolResult = tosVersionProtocol.parseTos(networkExpert.connect(requestCreateTos));
                } catch (NetworkExpertException e) {
                    if (vuParams.isCountLimit) {
                        this._runner.putFailure(new ModelErrorInfo(ModelErrorInfo.Type.NETWORK_FAILED, e));
                    }
    ...
    private static class FunctionProviderCage implements ModelGateway.FunctionProvider {
        ...
        public void create(ModelContext modelContext) {
            ...
            this.mfiCardFunc = new MfiCardFuncEntity(modelContext);
            this.cardDetailFunc = new CardDetailFuncEntity(modelContext);
            this.termsOfServiceFunc = new TermsOfServiceFuncEntity(modelContext);
            this.deleteCardListFunc = new DeleteCardListFuncEntity(modelContext);
        }
    ...
    private static class VuParams {
        ...
        public boolean setup(DatabaseExpert.UpgradeSettings upgradeSettings) {
            if (LAST_CHECK_DATE.equals(upgradeSettings.lastChkDate)) {
            ...
            Calendar calendarConvertFromStringToCalendar = FuncUtil.convertFromStringToCalendar("yyyyMMddHHmm", upgradeSettings.lastChkDate);
            ...
            if (((long) upgradeSettings.interval) + timeInMillis2 >= timeInMillis) {
