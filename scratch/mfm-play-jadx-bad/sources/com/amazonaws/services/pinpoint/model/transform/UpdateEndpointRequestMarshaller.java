...
public class UpdateEndpointRequestMarshaller implements Marshaller<Request<UpdateEndpointRequest>, UpdateEndpointRequest> {
    ...
    public Request<UpdateEndpointRequest> marshall(UpdateEndpointRequest updateEndpointRequest) {
        ...
        defaultRequest.setResourcePath("/v1/apps/{application-id}/endpoints/{endpoint-id}".replace("{application-id}", updateEndpointRequest.getApplicationId() == null ? "" : StringUtils.fromString(updateEndpointRequest.getApplicationId())).replace("{endpoint-id}", updateEndpointRequest.getEndpointId() != null ? StringUtils.fromString(updateEndpointRequest.getEndpointId()) : ""));
        try {
            StringWriter stringWriter = new StringWriter();
            AwsJsonWriter jsonWriter = JsonUtils.getJsonWriter(stringWriter);
            if (updateEndpointRequest.getEndpointRequest() != null) {
                EndpointRequestJsonMarshaller.getInstance().marshall(updateEndpointRequest.getEndpointRequest(), jsonWriter);
            }
            jsonWriter.close();
