...
public class PINChangeData extends Data {
    ...
    public PINChangeData(long j, long j2) throws IllegalArgumentException {
    ...
    public void setNewPIN(long j) throws IllegalArgumentException {
        LogMgr.log(4, "%s : newPIN = %d", "000", Long.valueOf(j));
        if (j < MIN_PIN || j > MAX_PIN) {
            LogMgr.log(1, "%s : pin = %d, Throw IllegalArgumentException EXC_PIN", "800", Long.valueOf(j));
            throw new IllegalArgumentException(EXC_PIN);
        }
        this.newPIN = j;
        LogMgr.log(4, "%s : this.newPIN = %d", "999", Long.valueOf(this.newPIN));
    }
    ...
    public void checkFormat() throws IllegalArgumentException {
        ...
        long j = this.pin;
        if (j < MIN_PIN || j > MAX_PIN) {
            LogMgr.log(6, "%s : pin = %d, Throw IllegalArgumentException EXC_PIN", "800", Long.valueOf(this.pin));
            throw new IllegalArgumentException(EXC_PIN);
        }
        long j2 = this.newPIN;
        if (j2 < MIN_PIN || j2 > MAX_PIN) {
            LogMgr.log(6, "%s : pin = %d, Throw IllegalArgumentException EXC_PIN", "801", Long.valueOf(this.newPIN));
            throw new IllegalArgumentException(EXC_PIN);
        }
        LogMgr.log(4, "%s", "999");
    }
