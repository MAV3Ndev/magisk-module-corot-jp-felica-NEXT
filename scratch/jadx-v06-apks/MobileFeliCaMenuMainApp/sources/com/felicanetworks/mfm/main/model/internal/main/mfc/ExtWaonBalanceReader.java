...
public class ExtWaonBalanceReader extends ExtFelicaReader {
    ...
    public MfcAdapterExpert.Asset getBalance(Data[] dataArr) {
        try {
            ...
            byte[] byteData = getByteData(dataArr[2]);
            if (((byteData[4] >>> 6) & 3) == 0 || ((byteData[0] >>> 7) & 1) == 1 || ((getByteData(dataArr[1])[12] >>> 7) & 1) == 1) {
                return asset;
            }
            asset.balanceValue = getPurseData(dataArr[0]);
            asset.point1 = new BigInteger(1, getTargetByte(getByteData(dataArr[3]), 0, 3)).intValue();
            return asset;
