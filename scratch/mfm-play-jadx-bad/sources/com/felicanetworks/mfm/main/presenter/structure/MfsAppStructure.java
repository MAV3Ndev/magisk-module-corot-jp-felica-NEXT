...
public class MfsAppStructure extends ExternalAppStructure {
    ...
    public MfsAppStructure(TermsOfServiceFuncAgent tos_agent) {
        super(StructureType.MFS_APP);
        this._tosAgent = tos_agent;
    }
    ...
    public void actResult(int resultCode, Intent data) {
        try {
            ...
            int intExtra = data.getIntExtra(RESULT_KEY_SETTINGAPP_FELICA, -1);
            if (intExtra == 0) {
                StateController.postStateEvent(StateMachine.Event.EV_RESULT_FELICA_SETTING_INCOMPLETE, this, new Object[0]);
                return;
            }
    ...
    public boolean confirmExistMfs(PackageManager pm) {
        int applicationEnabledSetting;
        try {
            applicationEnabledSetting = pm.getApplicationEnabledSetting((String) Sg.getValue(Sg.Key.SETTING_LAUNCH_LINK_APP_PKG_MFS));
    ...
    public boolean confirmSignMfs(PackageManager pm) {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= 33) {
                packageInfo = pm.getPackageInfo(str, PackageManager.PackageInfoFlags.of(134217728L));
            } else if (Build.VERSION.SDK_INT >= 28) {
                packageInfo = pm.getPackageInfo(str, 134217728);
            } else {
                packageInfo = pm.getPackageInfo(str, 64);
            }
