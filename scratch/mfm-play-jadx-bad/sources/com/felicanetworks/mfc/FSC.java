...
public class FSC extends Service {
    ...
    private IFSCEventListener ifscEventHooker = new IFSCEventListener.Stub() { // from class: com.felicanetworks.mfc.FSC.1
        ...
        public void finished(int status) {
            ...
            try {
                ...
                if (fSCEventListener == null) {
                    ...
                    LogMgr.log(7, "%s", "003");
                    LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "finished");
                    fSCEventListener.finished(status);
                    LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "finished");
                }
            } catch (Exception e) {
        ...
        public void errorOccurred(int type, String message) {
            ...
            try {
                ...
                if (fSCEventListener == null) {
                    ...
                    if (type == 100) {
                        LogMgr.log(7, "%s", "004");
                        LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "errorOccurred");
                        fSCEventListener.errorOccurred(type, message);
                        LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "errorOccurred");
                    } else {
                        LogMgr.log(7, "%s", "005");
                        LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "errorOccurred");
                        fSCEventListener.errorOccurred(type, message);
        ...
        public void operationRequested(int deviceID, String param, byte[] data) throws RemoteException {
            ...
            try {
                LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "operationRequested");
                byte[] bArrOperationRequested = FSC.this.fscEventListener.operationRequested(deviceID, param, data);
                LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSCEventListener", "operationRequested");
                LogMgr.log(3, "%s %s:%s", "020", "Client operation is completed", String.valueOf(bArrOperationRequested));
                try {
                    LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSC", "notifyResult");
                    FSC.this.fsc.notifyResult(bArrOperationRequested);
                    LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSC", "notifyResult");
                ...
                try {
                    LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSC", "notifyError");
                    FSC.this.fsc.notifyError(e.getMessage());
                    LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSC", "notifyError");
                    ...
                    try {
                        try {
                            if (this.fsc == null) {
                                ...
                                LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSC", LogSender.EXTRA_VALUE_EVENT_NAME_START, "url = " + url);
                                MfcUtil.checkMfcResult(this.fsc.start(url, this.deviceList, this.ifscEventHooker, this.felica.getIFelica()));
                                LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSC", LogSender.EXTRA_VALUE_EVENT_NAME_START);
                            }
                            this.online = true;
                            LogMgr.log(3, "%s", "999");
                        } catch (FelicaException e) {
                            LogMgr.log(2, "%s %s id:%s type:%s", "710", "FelicaException", Integer.valueOf(e.getID()), Integer.valueOf(e.getType()));
                            throw e;
                        }
                    } catch (Exception e2) {
                        LogMgr.log(2, "%s %s msg:%s", "799", "Exception", e2.getMessage());
                        throw new FelicaException(1, 47);
                    }
                } catch (IllegalArgumentException e3) {
                    LogMgr.log(2, "%s %s msg:%s", "711", "IllegalArgumentException", e3.getMessage());
                    throw e3;
                }
            } catch (FelicaException e4) {
                LogMgr.log(2, "%s %s id:%s type:%s", "712", "FelicaException", Integer.valueOf(e4.getID()), Integer.valueOf(e4.getType()));
                throw e4;
            }
    ...
    public synchronized void stop() throws FelicaException {
        ...
        try {
            LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSC", "stop");
            this.fsc.stop();
            LogMgr.performanceOut(LogMgr.PERFORMANCE_MFC, "FSC", "stop");
            ...
            synchronized (FSC.this) {
                ...
                try {
                    if (FSC.this.canceled) {
                        LogMgr.log(7, "%s", "020");
                        fSCEventListener = FSC.this.fscEventListener;
                        listenerParameterConvExceptionToListenerParameter = FSC.this.new ListenerParameter(2, "Interrupted.");
                        ...
                        try {
                            ...
                            try {
                                ...
                                LogMgr.performanceIn(LogMgr.PERFORMANCE_MFC, "FSC", LogSender.EXTRA_VALUE_EVENT_NAME_START, "url = " + str3);
