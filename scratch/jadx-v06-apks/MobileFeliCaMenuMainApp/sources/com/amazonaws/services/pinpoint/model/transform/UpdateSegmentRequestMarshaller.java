...
public class UpdateSegmentRequestMarshaller implements Marshaller<Request<UpdateSegmentRequest>, UpdateSegmentRequest> {
    ...
    public Request<UpdateSegmentRequest> marshall(UpdateSegmentRequest updateSegmentRequest) {
        ...
        DefaultRequest defaultRequest = new DefaultRequest(updateSegmentRequest, "AmazonPinpoint");
        defaultRequest.setHttpMethod(HttpMethodName.PUT);
        defaultRequest.setResourcePath("/v1/apps/{application-id}/segments/{segment-id}".replace("{application-id}", updateSegmentRequest.getApplicationId() == null ? "" : StringUtils.fromString(updateSegmentRequest.getApplicationId())).replace("{segment-id}", updateSegmentRequest.getSegmentId() != null ? StringUtils.fromString(updateSegmentRequest.getSegmentId()) : ""));
        try {
            StringWriter stringWriter = new StringWriter();
            AwsJsonWriter jsonWriter = JsonUtils.getJsonWriter(stringWriter);
            if (updateSegmentRequest.getWriteSegmentRequest() != null) {
                WriteSegmentRequestJsonMarshaller.getInstance().marshall(updateSegmentRequest.getWriteSegmentRequest(), jsonWriter);
            }
            jsonWriter.close();
