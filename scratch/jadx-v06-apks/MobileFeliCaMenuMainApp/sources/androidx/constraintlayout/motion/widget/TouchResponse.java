    ...
    */
    ...
    void processTouchEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        ...
        if (action == 1) {
            ...
            if (i != -1) {
                this.mMotionLayout.getAnchorDpDt(i, progress, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
            ...
            if (f5 == 0.0f || f5 == 1.0f || this.mOnTouchUp == 3) {
                if (0.0f >= f5 || 1.0f <= f5) {
    ...
    float getProgressDirection(float dx, float dy) {
        ...
        if (this.mTouchDirectionX != 0.0f) {
            ...
            return (dx * this.mTouchDirectionX) / this.mAnchorDpDt[0];
        }
        float[] fArr2 = this.mAnchorDpDt;
        if (fArr2[1] == 0.0f) {
            fArr2[1] = 1.0E-7f;
        }
        return (dy * this.mTouchDirectionY) / this.mAnchorDpDt[1];
    }
    ...
    void scrollUp(float dx, float dy) {
        ...
        if (progress != 0.0f) {
            if ((this.mOnTouchUp != 3) && (progress != 1.0f)) {
                this.mMotionLayout.touchAnimateTo(this.mOnTouchUp, ((double) progress) >= 0.5d ? 1.0f : 0.0f, f);
            }
        }
    }
