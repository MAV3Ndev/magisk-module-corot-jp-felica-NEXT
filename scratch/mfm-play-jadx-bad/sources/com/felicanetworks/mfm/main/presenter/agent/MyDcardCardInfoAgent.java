...
public class MyDcardCardInfoAgent extends MyCardInfoAgent {
    ...
    MyDcardCardInfoAgent(MyDcardInfo service, MyCardInfo client) {
        MyDcardInfo.DcardType dcardType;
        super(client);
        if (client.isActiveForeground()) {
            dcardType = service.getDcardType();
        } else {
            dcardType = MyServiceDatabaseAccess.getInstance().getDcardType(client.getCardId());
        }
        this.dcardType = dcardType;
        MyServiceDatabaseAccess.getInstance().setDcardType(client.getCardId(), dcardType);
    }
    ...
    String getSpecialCardFaceImageUrl() {
        return super.getSpecialCardFaceImageUrl();
    }
