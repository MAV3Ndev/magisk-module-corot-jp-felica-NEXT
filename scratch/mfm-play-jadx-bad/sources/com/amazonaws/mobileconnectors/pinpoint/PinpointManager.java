...
public class PinpointManager {
    ...
    public PinpointManager(PinpointConfiguration pinpointConfiguration) {
        try {
            ...
            pinpointContext.setNotificationClient(notificationClientCreateClient);
            PinpointNotificationActivity.setNotificationClient(notificationClientCreateClient);
            if (pinpointConfiguration.getEnableEvents()) {
                AnalyticsClient analyticsClient = new AnalyticsClient(pinpointContext);
                this.analyticsClient = analyticsClient;
                pinpointContext.setAnalyticsClient(analyticsClient);
                SessionClient sessionClient = new SessionClient(pinpointContext);
                this.sessionClient = sessionClient;
                pinpointContext.setSessionClient(sessionClient);
            } else {
            ...
            if (pinpointConfiguration.getEnableTargeting()) {
                if (pinpointConfiguration.getExecutor() != null) {
                    this.targetingClient = new TargetingClient(pinpointContext, pinpointConfiguration.getExecutor());
                } else {
                    this.targetingClient = new TargetingClient(pinpointContext);
                }
                pinpointContext.setTargetingClient(this.targetingClient);
                notificationClientCreateClient.addDeviceTokenRegisteredHandler(new DeviceTokenRegisteredHandler() { // from class: com.amazonaws.mobileconnectors.pinpoint.PinpointManager.1
            ...
            if (region != null && !"us-east-1".equals(region.getName())) {
                pinpointContext.getPinpointServiceClient().setEndpoint(String.format("pinpoint.%s.amazonaws.com", region.getName()));
            }
