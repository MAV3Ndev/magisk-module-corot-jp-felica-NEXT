...
public class PINEnableData extends Data {
    public static final Parcelable.Creator<PINEnableData> CREATOR = new Parcelable.Creator<PINEnableData>() { // from class: com.felicanetworks.mfc.PINEnableData.1
        ...
        public PINEnableData createFromParcel(Parcel in) {
            LogMgr.log(4, "%s : in = %s", "000", in);
            LogMgr.log(4, "%s", "999");
            return new PINEnableData(in);
        }

        /* JADX DEBUG: Method merged with bridge method: newArray(I)[Ljava/lang/Object; */
        /* JADX WARN: Can't rename method to resolve collision */
        @Override // android.os.Parcelable.Creator
        public PINEnableData[] newArray(int size) {
            LogMgr.log(4, "%s : size = %d", "000", Integer.valueOf(size));
            LogMgr.log(4, "%s", "999");
            return new PINEnableData[size];
        }
    };
    ...
    public PINEnableData(long pin, boolean enabling) throws IllegalArgumentException {
        LogMgr.log(4, "%s : pin = %d, enabling = %s", "000", Long.valueOf(pin), Boolean.valueOf(enabling));
        setPIN(pin);
    ...
    public void setEnabling(boolean enabling) {
        LogMgr.log(6, "%s : enabling = %s", "000", Boolean.valueOf(enabling));
    ...
    private PINEnableData(Parcel in) {
        LogMgr.log(6, "%s : in = %s", "000", in);
        readFromParcel(in);
        LogMgr.log(6, "%s", "999");
    }
