...
class UtcDates {
    ...
    static String getDefaultTextInputHint(Resources resources, SimpleDateFormat simpleDateFormat) {
        ...
        if (Locale.getDefault().getLanguage().equals(Locale.KOREAN.getLanguage())) {
            pattern = pattern.replaceAll("d+", "d").replaceAll("M+", Service.SERVICE_TYPE_1_MFI).replaceAll("y+", "y");
        }
        return pattern.replace("d", string3).replace(Service.SERVICE_TYPE_1_MFI, string2).replace("y", string);
    }
