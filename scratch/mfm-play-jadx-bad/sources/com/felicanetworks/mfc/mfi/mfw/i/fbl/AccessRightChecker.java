...
public class AccessRightChecker implements AccessController {
    ...
    public void check(int category) throws AccessControllerException {
        LogMgr.log(4, "%s In type = %s : mPermit = %s", "000", Integer.valueOf(category), this.mPermit);
                    ...
                    switch (category) {
                        ...
                        case 51:
                            if (!permit.getMfiPermit().getFunctionCategory().isExistEmptySlot()) {
                                LogMgr.log(7, "820 category != CATEGORY_EXIST_EMPTY_SLOT");
                                throw new AccessControllerException(1);
                            }
                            break;
                        default:
    ...
    @Override // com.felicanetworks.mfc.mfi.felica.access_control.AccessController
    public void checkSystemCode(int systemCode) throws AccessControllerException {
        LogMgr.log(4, "%s In systemCode = %s : mPermit = %s", "000", Integer.valueOf(systemCode), this.mPermit);
        Iterator<OfflineAccessRange> it = this.mPermit.getMfcPermit().getOfflineAccessRangeList().iterator();
        while (it.hasNext()) {
            int systemCode2 = it.next().getSystemCode();
            if (systemCode2 == 65535 || systemCode2 == systemCode) {
                LogMgr.log(4, "%s", "999");
                return;
            }
        }
    ...
    @Override // com.felicanetworks.mfc.mfi.felica.access_control.AccessController
    public void checkNodeCodeList(int currentSystemCode, int[] nodeCodeList) throws AccessControllerException {
        int i;
        LogMgr.log(4, "%s In currentSystemCode = %s : nodeCodeList = %s : mPermit = %s", "000", Integer.valueOf(currentSystemCode), nodeCodeList, this.mPermit);
        ...
        for (OfflineAccessRange offlineAccessRange : this.mPermit.getMfcPermit().getOfflineAccessRangeList()) {
            ...
            if (currentSystemCode == systemCode || 65535 == systemCode) {
                LogMgr.log(7, "%s", "001");
    ...
    @Override // com.felicanetworks.mfc.mfi.felica.access_control.AccessController
    public void checkMfiServiceId(String serviceId) throws AccessControllerException {
        LogMgr.log(4, "000 : In serviceId = " + serviceId + " : mPermit = " + this.mPermit);
        Permit permit = this.mPermit;
        if (permit == null) {
