...
public class DcmxMiniBalanceReader extends BalanceReader {
    ...
    private void setIssuerCodePatternNo(DcmxMiniDataItem dcmxMiniDataItem) throws FelicaReaderException {
        ...
        dcmxMiniDataItem.issureCode = (new BigInteger(getTargetByte(bArrChangeEndian, iArr[0], iArr[1])).intValue() >>> 7) & MASK_VALUE_ISSUERCODE;
        int[] iArr2 = GET_INFO_PATTERNNO;
        dcmxMiniDataItem.patternNoCode = (new BigInteger(getTargetByte(bArrChangeEndian, iArr2[0], iArr2[1])).intValue() >>> 1) & MASK_VALUE_PATTERNNO;
    }
    ...
    private void setMouthlyCrearDateOffLineValueLimit(DcmxMiniDataItem dcmxMiniDataItem) throws FelicaReaderException {
        ...
        dcmxMiniDataItem.monthlyClearDate_DD = CommonUtil.binToHexString(getTargetByte(bArrChangeEndian, iArr[0], iArr[1]));
        int[] iArr2 = GET_INFO_OFFLINEVALUELIMIT;
        dcmxMiniDataItem.offlineLimitValue = CommonUtil.binToHexString(getTargetByte(bArrChangeEndian, iArr2[0], iArr2[1]));
    }
    ...
    private void setCardAplArea(DcmxMiniDataItem dcmxMiniDataItem) throws FelicaReaderException {
        ...
        dcmxMiniDataItem.cardAplArea = CommonUtil.binToHexString(getTargetByte(bArrChangeEndian, iArr[0], iArr[1]));
    }
    ...
    private void setEnableFlgLastOfflineDateOfflineUseValue(DcmxMiniDataItem dcmxMiniDataItem) throws FelicaReaderException {
        ...
        dcmxMiniDataItem.enableFlag = CommonUtil.binToHexString(getTargetByte(bArrChangeEndian, iArr[0], iArr[1]));
        int[] iArr2 = GET_INFO_LASTOFFLINEDATE;
        dcmxMiniDataItem.latestOfflineDate_YYMMDD = CommonUtil.binToHexString(getTargetByte(bArrChangeEndian, iArr2[0], iArr2[1]));
        int[] iArr3 = GET_INFO_OFFLINEUSEVALUE;
        dcmxMiniDataItem.offlineUseValue = CommonUtil.binToHexString(getTargetByte(bArrChangeEndian, iArr3[0], iArr3[1]));
    }
    ...
    private void setBalanceValueLimit(MfcExpert.Asset asset, DcmxMiniDataItem dcmxMiniDataItem) {
        ...
        if (str2.compareTo(OFFSET_YY + dcmxMiniDataItem.latestOfflineDate_YYMMDD) < 0) {
            return;
        }
