...
public class FelicaSeController extends ChipController {
    private static final byte[] RESET_COMMAND = {2, -88};
    private static final byte[] RESET_RESPONSE = {2, -87};
    private static final int RESET_TIMEOUT = 1000;
    private IFelicaSe mChip;

    @Override // com.felicanetworks.mfc.felica.offlineimpl.ChipController
    public void open() throws OfflineException {
        try {
            this.mChip = getFelicaSe();
                ...
                try {
                    try {
                        try {
                            try {
                                LogMgr.log(3, "%s", "001");
                                LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "open");
                                Bundle bundleOpen = this.mChip.open(MFC_PACKAGE_NAME, this.mToken);
                                LogMgr.performanceOut(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "open");
                                LogMgr.log(3, "%s", "002");
    ...
    public void close(boolean z) {
        if (this.mChip != null) {
            ...
            try {
                try {
                    LogMgr.log(3, "%s", "001");
                    LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "close");
                    this.mChip.close(MFC_PACKAGE_NAME, this.mOpenHandle, this.mToken);
                    LogMgr.performanceOut(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "close");
                    LogMgr.log(3, "%s", "002");
                } finally {
                    unregisterBinder();
                    this.mChip = null;
    ...
    public void connect(int i) throws OfflineException {
        try {
            LogMgr.log(3, "%s", "001");
            LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "connect");
            int iConnect = this.mChip.connect(MFC_PACKAGE_NAME, this.mOpenHandle);
            LogMgr.performanceOut(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "connect");
            LogMgr.log(3, "%s", "002");
            if (iConnect < 0) {
                LogMgr.log(1, "%s", "702");
    ...
    public void disconnect() throws OfflineException {
        try {
            try {
                LogMgr.log(3, "%s", "001");
                LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "disconnect");
                this.mChip.disconnect(MFC_PACKAGE_NAME, this.mOpenHandle);
                LogMgr.performanceOut(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "disconnect");
                LogMgr.log(3, "%s", "002");
            } catch (RemoteException unused) {
                LogMgr.log(1, "%s", "700");
                throw new OfflineException(0);
                ...
                try {
                    try {
                        try {
                            setProtectCancel(z2);
                            if (z) {
                                LogMgr.logArray(4, RESET_COMMAND);
                                LogMgr.log(3, "%s", "001");
                                LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "transceive", "command len = " + RESET_COMMAND.length);
                                Bundle bundleTransceive = this.mChip.transceive(MFC_PACKAGE_NAME, this.mOpenHandle, RESET_COMMAND, 1000);
                                LogMgr.performanceOut(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "transceive");
                                LogMgr.log(3, "%s", "002");
                                byte[] byteArray = bundleTransceive.getByteArray("out");
                                if (byteArray == null) {
                                    LogMgr.log(1, "%s", "return value not found!");
                                    throw new OfflineException(convertNfcErrorToOfflineException(bundleTransceive.getInt("e")));
                                }
                                LogMgr.logArray(4, byteArray);
                                if (!compareData(byteArray, RESET_RESPONSE)) {
    ...
    protected Response doExecution(Command command, int i) throws OfflineException {
        try {
            ...
            LogMgr.log(3, "%s", "001");
            LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "transceive", "command len = " + bArr.length);
    ...
    public void cancelOffline() {
        ...
        if (this.mChip != null) {
            try {
                LogMgr.log(3, "%s", "001");
                LogMgr.performanceIn(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "cancel");
                this.mChip.cancel(MFC_PACKAGE_NAME, this.mOpenHandle);
                LogMgr.performanceOut(LogMgr.PERFORMANCE_LMW, "IFelicaSe", "cancel");
                LogMgr.log(3, "%s", "002");
    ...
    private IFelicaSe getFelicaSe() throws FeliCaChipException {
        IFelicaSe felicaSeService = this.mAdapter.getFelicaSeService();
        if (felicaSeService == null && (felicaSeService = this.mAdapter.getFelicaSeService()) == null) {
            throw new FeliCaChipException(0);
        }
        return felicaSeService;
    }
}
