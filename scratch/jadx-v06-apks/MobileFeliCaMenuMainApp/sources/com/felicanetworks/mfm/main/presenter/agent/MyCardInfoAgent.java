...
public abstract class MyCardInfoAgent extends InfoAgent {
        ...
        switch (AnonymousClass2.$SwitchMap$com$felicanetworks$mfm$main$policy$service$SupportServiceType[SupportServiceType.resolve(str).ordinal()]) {
            ...
            case 7:
                if (MyCardInfo.SupportMfiServiceType.UNSUPPORTED_MFI_SERVICE_1 != this._client.getUnsupportedMfiService1()) {
                    return MyCardInfo.SupportMfiServiceType.SUPPORTED_MFI_SERVICE_1 == this._client.getUnsupportedMfiService1() ? getCardStatusForFullIF() : cardStatusForCardDetailUI;
                }
                if (this._client.isActiveForeground()) {
                    return CardStatusForCardDetailUI.ACTIVE;
                }
                return this._client.isActiveBackground() ? CardStatusForCardDetailUI.INACTIVE : cardStatusForCardDetailUI;
            case 8:
    ...
    public boolean isShowCardDeposit(String str) {
        ...
        if (i == 7) {
            if (MyCardInfo.SupportMfiServiceType.SUPPORTED_MFI_SERVICE_1 == this._client.getUnsupportedMfiService1() && (this._client.isActiveForeground() || this._client.isActiveBackground())) {
                return true;
            }
        } else if (i == 8 && (this._client.isActiveForeground() || this._client.isActiveBackground())) {
            return true;
        }
        return false;
    }
    ...
    public static class Factory {
        public static MyCardInfoAgent create(MyServiceInfo myServiceInfo, MyCardInfo myCardInfo) {
            if (myServiceInfo instanceof MySuicaInfo) {
                return new MySuicaCardInfoAgent((MySuicaInfo) myServiceInfo, myCardInfo);
            }
            if (myServiceInfo instanceof MyDcardInfo) {
                return new MyDcardCardInfoAgent((MyDcardInfo) myServiceInfo, myCardInfo);
            }
            return new MyFavoriteCardInfoAgent(myCardInfo);
        }
    }
}
