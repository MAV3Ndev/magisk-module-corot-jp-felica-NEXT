...
public class CachedCard {
    ...
    public synchronized void enable(CachedCardEnableEventCallback callback) throws IllegalArgumentException, MfiClientException {
        LogMgr.log(3, "000");
        if (callback == null) {
        ...
        try {
            ...
            LogMgr.log(2, "702 " + e3.toString() + " id:" + e3.getID() + " type:" + e3.getType());
            this.mMfiClientAccess.stopOnline();
    ...
    class LocalICachedCardEnableEventCallback extends ICachedCardEnableEventCallback.Stub {
                    ...
                    try {
                        ...
                        try {
                            if (baseMfiEventCallback instanceof CachedCardEnableEventCallback) {
                                ((CachedCardEnableEventCallback) baseMfiEventCallback).onError(200, "Unknown error.");
                            }
        ...
        public void onError(int type, String msg) throws RemoteException {
            try {
                BaseMfiEventCallback baseMfiEventCallbackStopOnline = CachedCard.this.mMfiClientAccess.stopOnline();
                if (baseMfiEventCallbackStopOnline instanceof CachedCardEnableEventCallback) {
                    ((CachedCardEnableEventCallback) baseMfiEventCallbackStopOnline).onError(type, msg);
                }
            } catch (Exception e) {
