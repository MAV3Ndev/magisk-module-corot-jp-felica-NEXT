...
public final class LongSaturatedMathKt {
    ...
    public static final long m2120saturatingAddNuflL3o(long j, DurationUnit unit, long j2) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long jM2060toLongimpl = Duration.m2060toLongimpl(j2, unit);
        if (((j - 1) | 1) == Long.MAX_VALUE) {
            return m2119checkInfiniteSumDefinedPjuGub4(j, j2, jM2060toLongimpl);
        }
        if (((jM2060toLongimpl - 1) | 1) == Long.MAX_VALUE) {
            return m2121saturatingAddInHalvesNuflL3o(j, unit, j2);
        }
        long j3 = j + jM2060toLongimpl;
        return ((j ^ j3) & (jM2060toLongimpl ^ j3)) < 0 ? j < 0 ? Long.MIN_VALUE : Long.MAX_VALUE : j3;
    }

    /* JADX INFO: renamed from: checkInfiniteSumDefined-PjuGub4, reason: not valid java name */
    private static final long m2119checkInfiniteSumDefinedPjuGub4(long j, long j2, long j3) {
        if (!Duration.m2046isInfiniteimpl(j2) || (j ^ j3) >= 0) {
            return j;
        }
    ...
    private static final long m2121saturatingAddInHalvesNuflL3o(long j, DurationUnit durationUnit, long j2) {
        long jM2024divUwyO8pc = Duration.m2024divUwyO8pc(j2, 2);
        long jM2060toLongimpl = Duration.m2060toLongimpl(jM2024divUwyO8pc, durationUnit);
        return (1 | (jM2060toLongimpl - 1)) == Long.MAX_VALUE ? jM2060toLongimpl : m2120saturatingAddNuflL3o(m2120saturatingAddNuflL3o(j, durationUnit, jM2024divUwyO8pc), durationUnit, Duration.m2049minusLRDsOJo(j2, jM2024divUwyO8pc));
    }

    private static final long infinityOfSign(long j) {
        return j < 0 ? Duration.INSTANCE.m2111getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m2110getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long j, long j2, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (j2 - 1)) == Long.MAX_VALUE) {
            return Duration.m2065unaryMinusUwyO8pc(infinityOfSign(j2));
        }
    ...
    private static final long saturatingFiniteDiff(long j, long j2, DurationUnit durationUnit) {
        ...
        if (((j3 ^ j) & (~(j3 ^ j2))) < 0) {
            if (durationUnit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m2050plusLRDsOJo(DurationKt.toDuration(j4, DurationUnit.MILLISECONDS), DurationKt.toDuration(j5, durationUnit));
            }
            return Duration.m2065unaryMinusUwyO8pc(infinityOfSign(j3));
        }
