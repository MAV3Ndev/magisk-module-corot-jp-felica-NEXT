...
public class JwsObject {
    ...
    public JwsObject(String jwsStr) throws JwsException {
        LogMgr.log(4, "%s", "000");
        if (jwsStr == null) {
        ...
        try {
            ...
            LogMgr.log(1, "%s %s", "801", str);
            LogMgr.printStackTrace(7, e);
            throw new JwsException(0, str);
        }
    }
    ...
    public void verify(Map<String, String> keyMap) throws JwsException {
        LogMgr.log(4, "%s", "000");
        if (keyMap == null) {
        ...
        try {
            try {
                ...
                try {
                    ...
                    String str5 = map.get(string2);
                    String str6 = this.mJwsStr;
                    String strSubstring = str6.substring(0, str6.lastIndexOf(46));
                    ...
                    String str7 = e.getClass().getSimpleName() + ":" + e.getMessage();
                    LogMgr.log(1, "%s %s", "807", str7);
                    throw new JwsException(1, str7);
                }
            } catch (Exception e2) {
                String str8 = e2.getClass().getSimpleName() + ":" + e2.getMessage();
                LogMgr.log(1, "%s %s", "809", str8);
                throw new JwsException(0, str8);
            }
    ...
    public boolean checkExpirationTime(long spareSec) {
        LogMgr.log(4, "%s", "000");
        try {
            long jOptLong = this.mHeader.optLong(NAME_EXPIRATION_TIME, -1L) - spareSec;
            LogMgr.log(4, "%s", "999");
    ...
    private boolean checkSpecifyExpirationTime(long expirationTime) {
        long jCurrentTimeMillis = System.currentTimeMillis() / 1000;
        if (expirationTime >= jCurrentTimeMillis) {
            return true;
        }
        LogMgr.log(6, "%s Expiration time is invalid. :%d < %d", "500", Long.valueOf(expirationTime), Long.valueOf(jCurrentTimeMillis));
        return false;
    }
