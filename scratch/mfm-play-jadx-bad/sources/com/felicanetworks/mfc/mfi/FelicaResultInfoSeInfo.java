...
public class FelicaResultInfoSeInfo extends FelicaResultInfoType<SeInfo> implements Parcelable {
    public static final Parcelable.Creator<FelicaResultInfoSeInfo> CREATOR = new Parcelable.Creator<FelicaResultInfoSeInfo>() { // from class: com.felicanetworks.mfc.mfi.FelicaResultInfoSeInfo.1
        ...
        public FelicaResultInfoSeInfo createFromParcel(Parcel in) {
            LogMgr.log(4, "%s : in = %s", "000", in);
            LogMgr.log(4, "%s", "999");
            return new FelicaResultInfoSeInfo(in);
        }

        /* JADX DEBUG: Method merged with bridge method: newArray(I)[Ljava/lang/Object; */
        /* JADX WARN: Can't rename method to resolve collision */
        @Override // android.os.Parcelable.Creator
        public FelicaResultInfoSeInfo[] newArray(int size) {
            LogMgr.log(4, "%s : size = %d", "000", Integer.valueOf(size));
            LogMgr.log(4, "%s", "999");
            return new FelicaResultInfoSeInfo[size];
        }
    };
    ...
    public FelicaResultInfoSeInfo(SeInfo value) {
        super(value);
        LogMgr.log(4, "%s value=%s", "000", value);
        LogMgr.log(4, "%s", "999");
    }

    public FelicaResultInfoSeInfo(int exceptionType, String message) {
        super(exceptionType, message);
        LogMgr.log(4, "%s exceptionType=%d message=%s", "000", Integer.valueOf(exceptionType), message);
        LogMgr.log(4, "%s", "999");
    }

    public FelicaResultInfoSeInfo(int exceptionType, String message, int id, int type) {
    ...
    public FelicaResultInfoSeInfo(int exceptionType, String message, int id, int type, int statusFlag1, int statusFlag2) {
    ...
    protected void readFromParcel(Parcel in) {
        ...
        if (Build.VERSION.SDK_INT >= 33) {
            this.value = in.readParcelable(SeInfo.class.getClassLoader(), SeInfo.class);
        } else {
            this.value = in.readParcelable(SeInfo.class.getClassLoader());
        }
        LogMgr.log(6, "999");
    }

    private FelicaResultInfoSeInfo(Parcel in) {
        super(null);
        LogMgr.log(6, "%s : in = %s", "000", in);
        readFromParcel(in);
        LogMgr.log(6, "999");
