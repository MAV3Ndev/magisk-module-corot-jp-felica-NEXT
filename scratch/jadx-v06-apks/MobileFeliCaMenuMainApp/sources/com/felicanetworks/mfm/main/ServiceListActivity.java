...
public class ServiceListActivity extends BaseActivity implements TutorialOverviewFragment.Action, TermsOfServiceFragment.Action, TutorialAccountFragment.Action, TutorialCardReadingFragment.Action, TutorialPushReceivingFragment.Action {
        ...
        switch (AnonymousClass8.$SwitchMap$com$felicanetworks$mfm$main$presenter$structure$StructureType[structure.getType().ordinal()]) {
            ...
            case 10:
                ...
                if (mfsAppStructure.confirmExistMfs(getPackageManager()) && mfsAppStructure.confirmSignMfs(getPackageManager())) {
                    startActivityForResult(mfsAppStructure.getDefaultIntent(), REQUEST_CODE_MFS_APP);
                    break;
                }
                break;
            case 11:
    ...
    private boolean isShowSplash(Intent intent) {
        return (intent.hasExtra(NotificationController.NOTIFICATION_ID_KEY) || intent.hasExtra(EXTRA_KEY_REBOOT)) ? false : true;
    }
    ...
    public void showInitializeFatalError(int i) {
        ...
        customDialogFragmentCreateSingleChoiceDialog.setText(getString(i));
    ...
    @Override // com.felicanetworks.mfm.main.view.views.TermsOfServiceFragment.Action
    public void onFailedLoading(MfmException mfmException) {
        if (!(getCurrentRequest() instanceof TermsOfServiceDrawStructure)) {
            finish();
        } else {
            ((TermsOfServiceDrawStructure) getCurrentRequest()).actLoadingFailed(mfmException);
        }
    }

    @Override // com.felicanetworks.mfm.main.view.views.TermsOfServiceFragment.Action
    public void onServerError(MfmException mfmException) {
        if (!(getCurrentRequest() instanceof TermsOfServiceDrawStructure)) {
            finish();
        } else {
            ((TermsOfServiceDrawStructure) getCurrentRequest()).actServerError(mfmException);
        }
    }
    ...
    public void onCompleteTutorialAccount() {
        ...
        startActivityForResult(mfiLoginDrawStructure.getDefaultIntent(), REQUEST_CODE_MFI_LOGIN);
        mfiLoginDrawStructure.setLoginErrorResultListener(new MfiLoginDrawStructure.LoginErrorResultListener() { // from class: com.felicanetworks.mfm.main.ServiceListActivity.7
            ...
            public void onErrorResult(MfiLoginResultCode mfiLoginResultCode, Intent intent) {
                Locale locale = Locale.US;
                Object[] objArr = new Object[3];
                objArr[0] = "04";
                objArr[1] = Integer.valueOf(mfiLoginResultCode.getCode());
                objArr[2] = Integer.valueOf(intent != null ? intent.getIntExtra("code", 0) : 0);
                String str = String.format(locale, "%n[%s.%02d.%03d]", objArr);
    ...
    public void onCompleteTutorialCardReading(boolean z) {
        if (!(getCurrentRequest() instanceof TutorialSettingsDrawStructure)) {
            finish();
        } else {
            ((TutorialSettingsDrawStructure) getCurrentRequest()).setEnableCardReading(z);
            showFragmentWithSlide(new TutorialPushReceivingFragment());
        }
    }

    @Override // com.felicanetworks.mfm.main.view.views.TutorialPushReceivingFragment.Action
    public void onCompleteTutorialPushReceiving(boolean z) {
        if (!(getCurrentRequest() instanceof TutorialSettingsDrawStructure)) {
            finish();
        } else {
            ((TutorialSettingsDrawStructure) getCurrentRequest()).setEnablePushReceiving(z);
