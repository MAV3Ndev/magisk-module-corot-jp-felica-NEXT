...
public class MfiClientAccess {
    ...
    protected void bindMfiClientService() throws FelicaException {
        ...
        if (felica.getPackageManager().hasSystemFeature("android.hardware.type.watch")) {
            str = WEAR_MFI_SERVICE_PACKAGE_NAME;
        } else {
            str = "com.felicanetworks.mfm.main";
        }
        if (!SignatureUtil.checkAppCertHash(felica, "BE:51:DB:F4:FE:C8:9B:D3:28:46:45:7B:13:B7:30:08:76:AF:55:94:D2:87:4D:EE:02:69:04:96:5A:E4:A6:CB", str)) {
    ...
    synchronized void activate(String packageName, FelicaEventListener callback) throws FelicaException, IllegalArgumentException {
        LogMgr.log(7, "%s", "000");
        if (callback == null) {
    ...
    class BindTimerHandler extends Handler {
        ...
        void startTimer(int timeout) {
            LogMgr.log(7, "%s timeout=%d", "000", Integer.valueOf(timeout));
            if (timeout > 0) {
                LogMgr.log(7, "%s", "001");
                sendMessageDelayed(MfiClientAccess.this.mBindTimerHandler.obtainMessage(1), timeout);
            }
            LogMgr.log(7, "%s", "999");
        }
        ...
        public void handleMessage(Message msg) {
            ...
            LogMgr.log(7, "%s what=%d", "000", Integer.valueOf(msg.what));
            if (msg.what == 1) {
    ...
    class MfiConnection implements ServiceConnection {
        ...
        /* JADX DEBUG: Another duplicated slice has different insns count: {[]}, finally: {[CONST_STR, CONST_STR, INVOKE, CONST_STR, CONST_STR, INVOKE] complete} */
        /* JADX DEBUG: Don't trust debug lines info. Repeating lines: [608=4] */
        /* JADX DEBUG: Incorrect finally slice size: {[CONST_STR, CONST_STR, INVOKE, CONST_STR, CONST_STR, INVOKE, CONST_STR, CONST_STR, INVOKE, IGET, INVOKE, IGET, INVOKE, IGET, INVOKE, GOTO] complete}, expected: {[CONST_STR, CONST_STR, INVOKE, CONST_STR, CONST_STR, INVOKE] complete} */
        ...
        public void onServiceConnected(ComponentName arg0, IBinder arg1) {
            ...
            String message;
            LogMgr.log(7, "%s %s", "000", arg0.getClassName());
            synchronized (MfiClientAccess.this) {
                MfiClientAccess.this.mMfiFelica = IMfiFelica.Stub.asInterface(arg1);
                ...
                try {
                    if (MfiClientAccess.this.mCallback != null) {
                        try {
                            ...
                            LogMgr.log(7, "%s", "002");
                            LogMgr.log(7, "%s", "003");
                            felicaEventListener2 = MfiClientAccess.this.mCallback;
                            MfiClientAccess.this.mCallback = null;
                            MfiClientAccess.this.unbindMfiClient();
                            felicaEventListener = felicaEventListener2;
                        } catch (Exception e2) {
            ...
            if (felicaEventListener != null) {
                ...
                felicaEventListener.errorOccurred(i, message, otherAppInfo);
            }
            LogMgr.log(7, "%s", "999");
        }
            ...
            if (felicaEventListener != null) {
                try {
                    ...
                    if (otherAppInfo != null) {
                        LogMgr.log(7, "%s %s %d", "003", "FelicaEventListener#errorOccurred", Integer.valueOf(otherAppInfo.getPid()));
                    }
    ...
    public synchronized void startLoggingInOut(BaseMfiEventCallback callback, boolean checkOnline) throws MfiClientException {
        LogMgr.log(7, "%s", "000");
        if (this.mLoggingInOut) {
            BaseMfiEventCallback baseMfiEventCallback = this.mMfiLogInOutEventCallback;
            if ((baseMfiEventCallback instanceof StartEventCallback) || (baseMfiEventCallback instanceof StopEventCallback)) {
        ...
        this.mOnline = true;
        this.mMfiLogInOutEventCallback = callback;
        LogMgr.log(7, "%s", "999");
    }
    ...
    public synchronized void startOnline(BaseMfiEventCallback callback) throws MfiClientException {
        LogMgr.log(7, "000");
        onlineCheck();
        this.mOnline = true;
        this.mMfiOnlineEventCallback = callback;
        LogMgr.log(7, "999");
    }
    ...
    public synchronized void startOnlineWithoutLogin(BaseMfiEventCallback callback) throws MfiClientException {
        LogMgr.log(7, "000");
        if (this.mOnline) {
            LogMgr.log(2, "700 online processing");
    ...
    private static boolean isDataListEventSupportedVersion(String version) throws MfiClientException {
        if (version == null) {
            LogMgr.log(2, "700 Could not get MfiClient Version.");
            throw new MfiClientException(1, MfiClientException.TYPE_MFICLIENT_REMOTE_ACCESS_FAILED, null);
        }
        try {
            String[] strArrSplit = version.split(VERSION_DELIMITER);
