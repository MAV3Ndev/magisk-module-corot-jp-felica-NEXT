...
public class EventRecorder {
    ...
    JSONObject readEventFromCursor(Cursor cursor, HashMap<Integer, Integer> map) throws Throwable {
        ...
        try {
            try {
                ...
                numValueOf = Integer.valueOf(cursor.getInt(ID_COLUMN_INDEX));
                try {
                    if (cursor.isNull(SIZE_COLUMN_INDEX)) {
                        log.error("Column 'SIZE' for event was NULL.");
                        numValueOf2 = null;
                    } else {
                        numValueOf2 = Integer.valueOf(cursor.getInt(SIZE_COLUMN_INDEX));
                    }
                    try {
            ...
            do {
                ...
                for (Integer num : map.keySet()) {
                    try {
                        ...
                        log.error("Failed to delete event: " + num, e);
                    }
                }
                if (i >= jIntValue) {
                    break;
    ...
    private void submitEventsAndEndpoint(JSONArray jSONArray, HashMap<Integer, Integer> map, EndpointProfile endpointProfile) {
        ...
        try {
            ...
            if (isRetryable(errorCode)) {
                log.error(String.format("AmazonServiceException: Unable to successfully deliver events to server. Events will be saved, error is likely recoverable. Response Status code: %s, Response Error Code: %s", Integer.valueOf(e.getStatusCode()), e.getErrorCode()), e);
                map.clear();
                return;
            }
            log.error(String.format(Locale.getDefault(), "Failed to submit events to EventService: statusCode: " + e.getStatusCode() + " errorCode: ", errorCode), e);
            ...
            if (isClientExceptionRetryable(e2)) {
    ...
    private void processEndpointResponse(EndpointProfile endpointProfile, PutEventsResult putEventsResult) {
        EndpointItemResponse endpointItemResponse = putEventsResult.getEventsResponse().getResults().get(endpointProfile.getEndpointId()).getEndpointItemResponse();
    ...
    private void processEventsResponse(JSONArray jSONArray, EndpointProfile endpointProfile, PutEventsResult putEventsResult, Map<Integer, Integer> map) {
        for (int i = 0; i < jSONArray.length(); i++) {
            try {
                ...
                } else if (isRetryable(eventItemResponse.getMessage())) {
    ...
    private boolean isRetryable(String str) {
