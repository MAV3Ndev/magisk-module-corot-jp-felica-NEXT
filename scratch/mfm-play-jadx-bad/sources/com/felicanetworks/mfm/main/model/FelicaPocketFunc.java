...
public interface FelicaPocketFunc {
    ...
    public static class FpParser {
        ...
        public FpParser(MfcExpert.FpArea fpArea, ModelContext context) {
        ...
        public FpParser(MfcAdapterExpert.FpArea fpArea, ModelContext context) {
            this._context = context;
            this.fpNum = fpArea.getFpNum();
            this.totalPocket = fpArea.getTotalPocket();
        ...
        public FpService getFpService(int num) {
            return this.fpServiceHash.get(num);
        }
        ...
        public static class FpService {
            ...
            public FpService(byte[] fpServiceNumber, byte[] fpIndexArea) {
                this.fpServiceNum = fpServiceNumber;
                this.fpIndex = fpIndexArea;
            }
            ...
            private static long getUInt32b(byte[] bin, int offset) {
                return ((long) (bin[offset + 3] & 255)) | (((long) (bin[offset] & 255)) << 24) | ((long) ((bin[offset + 1] & 255) << 16)) | ((long) ((bin[offset + 2] & 255) << 8));
            }

            private static short getInt16b(byte[] bin, int offset) {
                return (short) ((bin[offset + 1] & 255) | ((bin[offset] & 255) << 8));
            }

            private static int convertIndexServiceNo(byte[] array, int offset) {
                int i = offset + 1;
                byte b = array[i];
                if (b == 0 && array[offset + 2] == 0) {
                    return 0;
                }
                if ((b & (-128)) != 0) {
                    return (array[offset] & 255) | ((b & 255) << 16) | ((array[offset + 2] & 255) << 8);
                }
                return getInt16b(array, i);
            }
        }
        ...
        private FpServiceInfo.Type getFpType(DatabaseExpert.FelicaPocketService dbService) {
            if (dbService != null) {
                if (dbService.serviceEnable == 0) {
                    return FpServiceInfo.Type.UNSUPPORTED;
                }
                if (dbService.isPointService() && dbService.needShowValue()) {
